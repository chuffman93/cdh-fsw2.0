...
FSWMode ScheduleManager::checkNewMode(){
	...
	Logger::Stream(LEVEL_DEBUG,tags) << "Current Mode: " << CurrentMode << "Current Schedule.mode(): "<< currentSchedule.mode << " Next Scheduled Mode: " << ScheduleQueue.front().mode << " Time since epoch: " << ScheduleQueue.front().sleepTime << " Time: " << time << " Enter time: " << modeEnterTime;
	...
	else if((currentSchedule.mode!=0) && currentSchedule.sleepTime <= time && CurrentMode != currentSchedule.mode){
...
void ScheduleManager::loadSchedule(std::string filePath){
	...
	if ((schedule.size()%9) != 0){
		...
		for (size_t i = 0; i < (schedule.size()/9); i++){
			bs >> mode >> sch.sleepTime >> sch.duration;
			int md = (int)mode;
			char log[100];
			sprintf(log,"Loading Schedule -- Mode: %u Time Since Epoch: %u Duration: %u ",mode,sch.sleepTime,sch.duration);
