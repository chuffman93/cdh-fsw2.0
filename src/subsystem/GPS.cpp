...
// set operator for printing out GPSPositionTime
...
GPS::GPS(NMEAInterface& nm, SubPowerInterface& pow):nm(nm), pow(pow),watchdog(0){
...
// Calcs the time to sleep by Propagating from the current ECI and checking the ECEF conversion
...
// used to get the saved GPSPositionTime
...
//CRC value check for GPS Authenticity
...
// resets lock upon the 2 hr limit
void GPS::resetLock(){
	isLocked = false;
}
